<?php
	...
	function new_player($eth_address) {
		...
		$q->bind_param ( 'sss', $eth_address, $empty, $empty);
	 ...
	 */
	function get_players($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_players $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_players WHERE id_player = ? LIMIT 1" );
