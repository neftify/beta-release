<?php
	...
	function new_interval($slp_generated_in_interval, $axies_on_pool_in_interval, $lenders_rate_in_interval, $players_rate_in_interval, $dao_rate_in_interval) {
		...
		$q = $db->prepare ( "INSERT INTO ngc_axie_interval_pools (slp_generated_in_interval, axies_on_pool_in_interval, lenders_rate_in_interval, players_rate_in_interval, dao_rate_in_interval, date_unix) VALUES (?, ?, ?, ?, ?, ?)" );
		$q->bind_param ( 'ssssss', $slp_generated_in_interval, $axies_on_pool_in_interval, $lenders_rate_in_interval, $players_rate_in_interval, $dao_rate_in_interval, $time);
	...
	function get_intervals($type, $query = '', $extra = '') {
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_axie_interval_pools $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_axie_interval_pools WHERE id_asset = ? LIMIT 1" );
