<?php
	...
	function new_axie_record($id_current_interval, $eth_address, $amount) {
		global $db;

		$q = $db->prepare ( "INSERT INTO ngc_lenders_axie_record (id_current_interval, eth_ronin_address, amount) VALUES (?, ?, ?)" );
		$q->bind_param ( 'sss', $id_current_interval, $eth_address, $amount);
	 ...
	 */
	function get_axie_records($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_lenders_axie_record $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_lenders_axie_record WHERE id_record = ? LIMIT 1" );
