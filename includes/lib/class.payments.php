<?php
	...
	function new_payment($eth_address, $id_lender, $id_player, $id_last_interval, $status, $amount, $token_symbol) {
		global $db;

		$empty = '';

		$q = $db->prepare ( "INSERT INTO ngc_users_payments (id_lender, id_player, id_last_interval, `status`, amount, token_symbol, eth_ronin_address) VALUES (?, ?, ?, ?, ?, ?, ?)" );
		$q->bind_param ( 'sssssss', $id_lender, $id_player, $id_last_interval, $status, $amount, $token_symbol, $eth_address);
	...
	function get_payments($type, $query = '', $extra = '') {
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_users_payments $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_users_payments WHERE id_payment = ? LIMIT 1" );
