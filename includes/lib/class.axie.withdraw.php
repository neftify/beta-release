<?php
	...
	function new_axie_withdraw($eth_address) {
		global $db;

        $empty_param = '';

		$q = $db->prepare ( "INSERT INTO ngc_axie_withdraw (eth_ronin_address, axie_id) VALUES (?, ?)" );
		$q->bind_param ( 'ss', $eth_address, $empty_param);
	 ...
	 */
	function get_axie_withdraw($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_axie_withdraw $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_axie_withdraw WHERE id_withdraw = ? LIMIT 1" );
