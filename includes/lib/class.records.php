<?php
	...
	function new_record($description, $value) {
		global $db;

		$q = $db->prepare ( "INSERT INTO ngc_record (short_description, `value`) VALUES (?, ?)" );
		$q->bind_param ( 'ss', $description, $value);
	 ...
	 */
	function get_ngc_record ($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM ngc_record $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM ngc_record WHERE id_record = ? LIMIT 1" );
